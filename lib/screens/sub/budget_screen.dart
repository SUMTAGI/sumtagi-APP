...
class _BudgetScreenState extends State<BudgetScreen> {
  ...
  final _catColors = {
    '교통': const Color(0xFFDBEAFE),
    '숙박': const Color(0xFFEDE9FE),
    '식사': const Color(0xFFFFEDD5),
    '체험': const Color(0xFFDCFCE7),
    '기타': AppColors.gray100,
  };
  final _catTextColors = {
    '교통': const Color(0xFF1D4ED8),
    '숙박': const Color(0xFF7C3AED),
    '식사': const Color(0xFFEA580C),
    '체험': const Color(0xFF16A34A),
    '기타': AppColors.gray700,
  };
  final _catIcons = {
    '교통': Icons.directions_boat_rounded,
    '숙박': Icons.hotel_rounded,
    '식사': Icons.restaurant_rounded,
    '체험': Icons.camera_alt_rounded,
    '기타': Icons.attach_money_rounded,
  };
  ...
  Future<void> _load() async {
    ...
    final stored = prefs.getString('budget');
    if (stored != null) {
      final data = jsonDecode(stored) as Map<String, dynamic>;
      setState(() {
        _totalBudget = (data['totalBudget'] as int?) ?? 500000;
  ...
  Future<void> _save() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('budget', jsonEncode({'totalBudget': _totalBudget, 'expenses': _expenses}));
  }
      ...
      appBar: AppBar(
        title: Column(
          ...
          children: [
            ...
            const Text('지출을 기록하고 예산을 관리하세요', style: TextStyle(fontSize: 11, color: AppColors.gray500)),
          ],
        ),
        backgroundColor: Colors.white,
              ...
              children: [
                Column(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        Expanded(
                          child: Text('${_totalBudget.toString().replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (m) => '${m[1]},')}원',
                ...
                ClipRRect(
                  ...
                  child: LinearProgressIndicator(
                    ...
                    color: totalExpense > _totalBudget ? const Color(0xFFF87171) : Colors.white,
                    minHeight: 8,
                  ),
                ),
              ],
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        TextField(
                          ...
                          decoration: InputDecoration(hintText: '10000', border: OutlineInputBorder(borderRadius: BorderRadius.circular(8)), filled: true, fillColor: Colors.white, contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10)),
                        ),
                        const SizedBox(height: 10),
                        ...
                        TextField(
                          ...
                          decoration: InputDecoration(hintText: '점심 식사', border: OutlineInputBorder(borderRadius: BorderRadius.circular(8)), filled: true, fillColor: Colors.white, contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10)),
                        ),
                        const SizedBox(height: 12),
                        Row(
                          children: [
                            Expanded(
                              child: ElevatedButton(onPressed: _addExpense, style: ElevatedButton.styleFrom(backgroundColor: AppColors.blue600, foregroundColor: Colors.white, elevation: 0, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))), child: const Text('추가')),
                            ),
                            const SizedBox(width: 8),
                            Expanded(
                              child: ElevatedButton(onPressed: () => setState(() => _showAddForm = false), style: ElevatedButton.styleFrom(backgroundColor: AppColors.gray200, foregroundColor: AppColors.gray700, elevation: 0, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))), child: const Text('취소')),
                            ),
                          ],
                        ),
                      ],
                  ...
                  Column(
                    children: [
                      ...
                      TextButton(
                        ...
                        child: const Text('첫 지출 추가하기', style: TextStyle(color: AppColors.blue600, fontWeight: FontWeight.w600)),
                      ),
                    ],
                  )
                else
                                ...
                                Row(
                                  ...
                                  children: [
                                    Column(
                                      crossAxisAlignment: CrossAxisAlignment.start,
                                      children: [
                                        Text(expense['description'] as String, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: AppColors.gray900)),
                                        const SizedBox(height: 2),
                                        Text(DateTime.tryParse(expense['date'] as String)?.toLocal().toString().substring(0, 16) ?? '', style: const TextStyle(fontSize: 11, color: AppColors.gray500)),
                                      ],
                                    ),
                                    GestureDetector(
