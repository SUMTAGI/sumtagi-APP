...
class ChecklistService {
  ...
  static Future<void> addItem({required String title, String category = '기타'}) async {
    if (_userId == null) return;
    final data = await _client.from('checklist_items').select('order_index').eq('user_id', _userId!).order('order_index', ascending: false).limit(1);
